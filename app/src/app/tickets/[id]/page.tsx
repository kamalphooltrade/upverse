    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {(t.status === "confirmed" || t.status === "sent") && <Card><H2>{t.rail === "manual" ? "ทำในแอป Webull แล้วกรอกผลจริง" : "รอผลจากโบรกเกอร์ — กรอก fill เองได้ถ้าเห็นในแอป"}</H2>
          {health?.broker?.status === "connected" && <div className="mb-3 rounded-2xl p-3 bg-gradient-to-r from-emerald-600/10 to-teal-500/10"><div className="text-[14px]">ทำในแอป Webull เสร็จแล้ว? กดปุ่มนี้ — ระบบดึง fill จาก Webull มาจับคู่กับตั๋วใบนี้และลงพอร์ตให้เอง (ไม่ต้องพิมพ์ราคา/จำนวน)</div><Btn className="mt-2" variant="primary" disabled={busy} onClick={() => act(() => api("/api/v1/broker/sync", { method: "POST" }), "ดึงจาก Webull แล้ว — ถ้าคำสั่งเสร็จแล้ว ตั๋วจะเปลี่ยนเป็น 'สำเร็จ' ด้านล่าง")}>ดึงจาก Webull แล้วจับคู่ fill อัตโนมัติ</Btn></div>}

...
import { Card, H2, Muted, Src, Chip, Btn, Delta, Toggle, KV, Skeleton, Banner, StarIcon, usd, fmtTime } from "@/components/ui";
...
export default function StockPage({ params }: { params: Promise<{ symbol: string }> }) {
  ...
  const { data, error } = useSWR<Inst>(`/api/v1/instruments/${symbol}?range=${range}`, fetcher, { refreshInterval: 15_000 });
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <Card><H2>ทำอะไรได้</H2>
          ...
          <Muted className="mt-2 text-[13px]">Thesis: ให้ agent `upverse-advisor` เขียนผ่าน API (theses:write) — ในเวอร์ชันนี้เก็บเป็นโน้ตใน journal</Muted>

...
export async function GET() {
  ...
  return json({
    ...
    broker: { configured: !!d.brokerCredentials, status: d.brokerCredentials?.status ?? "unset", token_status: d.brokerCredentials?.tokenStatus ?? null, region: d.brokerCredentials?.region ?? null, key_last4: d.brokerCredentials?.keyLast4 ?? null, master_key: masterKeyConfigured(), accounts: d.brokerCredentials?.accounts?.length ?? 0 },
    owner_passphrase_source: d.owner ? "db" : ownerConfigured() ? "env" : "none",

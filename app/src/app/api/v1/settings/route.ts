...
const S = z.object({ tradingEnabled: z.boolean().optional(), environment: z.enum(["uat", "prod"]).optional(), confirmProdPhrase: z.string().optional(), quietHours: z.object({ start: z.string(), end: z.string() }).optional(), hideAmountsInLine: z.boolean().optional(), theme: z.enum(["auto", "dark", "light"]).optional(), fxUsdThb: z.object({ rate: z.number().positive(), asOf: z.string(), source: z.string() }).nullable().optional(), fxSpreadPct: z.number().min(0).max(5).nullable().optional() });
